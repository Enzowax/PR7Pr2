...
Регрессий в 37 автотестах не выявлено.
...
2. **Ручное тестирование** обнаружило класс багов, недоступных автоматизированной проверке: юзабилити-проблемы (отсутствие keyboard accelerator), missing user feedback (тихий no-op), stale state (устаревшие данные на экране). Эти баги не имеют функциональных последствий, но снижают удобство и могут привести к ошибкам пользователя.
3. **Покрытие тестами по уровням пирамиды**:
   - Unit-tests (34) — алгоритм
   - Performance benchmarks (3) — производительность ядра
   - Manual NFR tests (50) — UX, надёжность, безопасность, локализация
4. После исправления всех пяти багов финальные прогоны:
   - `dotnet test` → 37/37 пройдено
   - Smoke-тест запуска приложения → успех
   - Повторная ручная проверка тест-кейсов BUG-005..BUG-007 → все исправления подтверждены
