...
| Поле | Значение |
| --- | --- |
| Разработчики | Новиков Дмитрий Евгеньевич, Соболенко Анна Романовна |
| Группа | 3ИСИП-423 |
...
Шифр Билайна (двухлинейный, в литературе чаще называемый Rail Fence или «шифр железнодорожной ограды») — классический симметричный шифр перестановки. Открытый текст записывается зигзагом по N горизонтальным линиям («рельсам»), после чего считывается по строкам — слева направо, сверху вниз.

Пример для строки `HELLOWORLD` при трех «рельсах»:
...
Подробное описание всех use-case с предусловиями, основными и альтернативными сценариями — в [docs/USE_CASE_DIAGRAM.md](docs/USE_CASE_DIAGRAM.md).
...
**Актор:** Пользователь (единственный).
...
**Вспомогательные:**
- **UC7.** Получить уведомление об ошибке (при невалидных данных)
...
Скриншоты Visual Studio (`breakpoints.png`, `locals.png`, `watch.png`, `call_stack.png`, `immediate.png`, `test_explorer.png`) в папке [docs/](docs/).
...
| --- | --- | --- | --- | --- | --- | --- |
| TC-01 | Классическое шифрование `HELLOWORLD` при 3 строках | `text=HELLOWORLD`, `rows=3` | `HOLELWRDLO` | Получено `HOLELWRDLO` | `Encrypt_ClassicHelloWorldWithThreeRows_ReturnsExpectedCipher` | Пройден |
| TC-02 | Обратимость для английского текста | `HELLOWORLD`, `rows=3` | `Decrypt(Encrypt(x)) == x` | `HELLOWORLD` восстановлен | `Decrypt_AfterEncryption_RestoresOriginalWithThreeRows` | Пройден |
| TC-03 | Обратимость для русского текста | «Поддержка и тестирование программных модулей», `rows=4` | `x` восстановлен | Совпало посимвольно | `EncryptDecrypt_RussianText_RestoresOriginal` | Пройден |
| TC-04 | Обратимость с пробелами и пунктуацией | `Hello, World! 2026.`, `rows=5` | `x` восстановлен | Совпало | `EncryptDecrypt_TextWithSpacesAndPunctuation_RestoresOriginal` | Пройден |
| TC-05 | Обратимость при rows=2 | `ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789`, `rows=2` | `Decrypt(Encrypt(x), 2) == x` | Совпало | `EncryptDecrypt_DifferentRowCounts_AreReversible (2)` | Пройден |
| TC-06 | Обратимость при rows=3 | то же, `rows=3` | `x` восстановлен | Совпало | `EncryptDecrypt_DifferentRowCounts_AreReversible (3)` | Пройден |
| TC-07 | Обратимость при rows=4 | то же, `rows=4` | `x` восстановлен | Совпало | `EncryptDecrypt_DifferentRowCounts_AreReversible (4)` | Пройден |
| TC-08 | Обратимость при rows=5 | то же, `rows=5` | `x` восстановлен | Совпало | `EncryptDecrypt_DifferentRowCounts_AreReversible (5)` | Пройден |
| TC-09 | Обратимость при rows=7 | то же, `rows=7` | `x` восстановлен | Совпало | `EncryptDecrypt_DifferentRowCounts_AreReversible (7)` | Пройден |
| TC-10 | Обратимость при rows=10 | то же, `rows=10` | `x` восстановлен | Совпало | `EncryptDecrypt_DifferentRowCounts_AreReversible (10)` | Пройден |
| TC-11 | Обратимость при rows=50 | то же, `rows=50` | `x` восстановлен | Совпало | `EncryptDecrypt_DifferentRowCounts_AreReversible (50)` | Пройден |
| TC-12 | Длинный текст 1000 символов | `'A'*500 + 'B'*500`, `rows=7` | `x` восстановлен | Совпало | `EncryptDecrypt_LongText_RestoresOriginal` | Пройден |
| TC-13 | Сохранение длины при шифровании | «Программирование на C# …», `rows=6` | `cipher.Length == text.Length` | 38 == 38 | `Encrypt_PreservesLength` | Пройден |
| TC-14 | Шифр отличается от исходного текста | «Тестовая фраза для шифрования», `rows=3` | `cipher != text` | Не равны | `Encrypt_ProducesDifferentTextThanOriginal` | Пройден |
| TC-15 | Разные rows дают разные шифротексты | `ZIGZAGCIPHER` при rows=2,3,5 | три разных результата | Все три различны | `Encrypt_SameTextWithDifferentRows_ProducesDifferentResults` | Пройден |
| TC-16 | Шифрование при rows=2 — чёт/нечёт | `ABCDEF`, `rows=2` | `ACEBDF` | Получено `ACEBDF` | `Encrypt_TwoRows_ProducesEvenOddSplit` | Пройден |
| TC-17 | Визуализация `HELLOWORLD` при 3 строках | `text=HELLOWORLD`, `rows=3` | сетка `H...O...L. / .E.L.W.R.D / ..L...O...` | Сетка совпадает посимвольно | `Visualize_ThreeRowsHelloWorld_ReturnsExpectedZigzagGrid` | Пройден |
| TC-18 | Визуализация содержит все символы исходного текста | `ABCDEFG`, `rows=3` | каждый символ присутствует | Все 7 символов найдены в выводе | `Visualize_ContainsAllOriginalCharacters` | Пройден |

### Краевые случаи

| № | Сценарий | Вход | Ожидаемый результат | Фактический результат | Тест / кейс | Статус |
| --- | --- | --- | --- | --- | --- | --- |
| TC-19 | rows == длине текста | `ABCDE`, `rows=5` | возвращает текст без изменений | `ABCDE` | `Encrypt_RowsEqualToTextLength_ReturnsTextUnchanged` | Пройден |
| TC-20 | rows > длины текста | `AB`, `rows=10` | текст без изменений | `AB` | `Encrypt_RowsGreaterThanTextLength_ReturnsTextUnchanged` | Пройден |
| TC-21 | Пустая строка для шифрования | `""`, `rows=3` | `""` | Получено `""` | `Encrypt_EmptyString_ReturnsEmptyString` | Пройден |
| TC-22 | Пустая строка для дешифрования | `""`, `rows=3` | `""` | Получено `""` | `Decrypt_EmptyString_ReturnsEmptyString` | Пройден |
| TC-23 | Пустая визуализация | `""`, `rows=3` | `""` | Получено `""` | `Visualize_EmptyString_ReturnsEmptyString` | Пройден |

### Негативные сценарии

| № | Сценарий | Вход | Ожидаемый результат | Фактический результат | Тест / кейс | Статус |
| --- | --- | --- | --- | --- | --- | --- |
| TC-24 | Шифрование null-текста | `text=null`, `rows=3` | `ArgumentNullException` | Брошено `ArgumentNullException` с message «Текст не может быть равен null.» | `Encrypt_NullText_ThrowsArgumentNullException` | Пройден |
| TC-25 | Дешифрование null-текста | `cipher=null`, `rows=3` | `ArgumentNullException` | Брошено `ArgumentNullException` | `Decrypt_NullText_ThrowsArgumentNullException` | Пройден |
| TC-26 | Encrypt: rows = 0 | `Test`, `rows=0` | `ArgumentOutOfRangeException` | Брошено: «должно находиться в диапазоне от 2 до 100» | `Encrypt_RowsOutOfRange_ThrowsArgumentOutOfRangeException (0)` | Пройден |
| TC-27 | Encrypt: rows = 1 | `Test`, `rows=1` | `ArgumentOutOfRangeException` | Брошено | `Encrypt_RowsOutOfRange_… (1)` | Пройден |
| TC-28 | Encrypt: rows = -3 | `Test`, `rows=-3` | `ArgumentOutOfRangeException` | Брошено | `Encrypt_RowsOutOfRange_… (-3)` | Пройден |
| TC-29 | Encrypt: rows = 101 | `Test`, `rows=101` | `ArgumentOutOfRangeException` | Брошено | `Encrypt_RowsOutOfRange_… (101)` | Пройден |
| TC-30 | Encrypt: rows = 1000 | `Test`, `rows=1000` | `ArgumentOutOfRangeException` | Брошено | `Encrypt_RowsOutOfRange_… (1000)` | Пройден |
| TC-31 | Decrypt: rows = 0 | `Test`, `rows=0` | `ArgumentOutOfRangeException` | Брошено | `Decrypt_RowsOutOfRange_… (0)` | Пройден |
| TC-32 | Decrypt: rows = 1 | `Test`, `rows=1` | `ArgumentOutOfRangeException` | Брошено | `Decrypt_RowsOutOfRange_… (1)` | Пройден |
| TC-33 | Decrypt: rows = -1 | `Test`, `rows=-1` | `ArgumentOutOfRangeException` | Брошено | `Decrypt_RowsOutOfRange_… (-1)` | Пройден |
| TC-34 | Decrypt: rows = 101 | `Test`, `rows=101` | `ArgumentOutOfRangeException` | Брошено | `Decrypt_RowsOutOfRange_… (101)` | Пройден |
...
Бенчмарки реализованы в [BeelineCipher.Tests/PerformanceBenchmark.cs](BeelineCipher.Tests/PerformanceBenchmark.cs) с категорией `[TestCategory("Performance")]`.
...
1. **Модульное тестирование (Unit testing)** — 34 автотеста на MSTest для модулей `Encrypt`, `Decrypt`, `Visualize`. Каждый тест проверяет один атомарный сценарий.
